...
router.get('/products', async (req: AuthRequest, res: Response) => {
  ...
  const { data: counts } = await supabase.from('bioedge_proof_corrections').select('product_id')
...
router.put('/products/:id', async (req: AuthRequest, res: Response) => {
  ...
  if (req.userLangs?.length) {
    const { data: existing } = await supabase.from('bioedge_proof_products').select('language').eq('id', req.params.id).single()
...
router.put('/corrections/:id', async (req: AuthRequest, res: Response) => {
  ...
  if (doneOnlyRoles.includes(role)) {
    ...
    const correction = await supabase
      .from('bioedge_proof_corrections').select('product_id').eq('id', req.params.id).single()
    if (correction.error) return res.status(500).json({ error: correction.error.message })

    const product = await supabase
      .from('bioedge_proof_products').select('ready_for_revision').eq('id', correction.data.product_id).single()
