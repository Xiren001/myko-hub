...
import { sendProofNotificationsForLanguage } from './proofNotifier'

export async function getDelayMinutes(): Promise<number> {
...
export async function enqueueNotification(language: string): Promise<void> {
  const delayMinutes = await getDelayMinutes()
  const scheduledFor = new Date(Date.now() + delayMinutes * 60 * 1000).toISOString()

  await supabase.from('proof_notification_queue').upsert(
  ...
  cron.schedule('* * * * *', async () => {
    try {
      ...
      for (const entry of due) {
        ...
        const result = await sendProofNotificationsForLanguage(entry.language)
