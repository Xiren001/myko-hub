...
export async function sendBioedgeNotificationsForLanguage(language: string): Promise<NotifyResult> {
  ...
  const subject = `BioEdge — ${count} product${count > 1 ? 's' : ''} waiting for ${language} proofread`
...
<div style="font-family:sans-serif;max-width:600px;margin:0 auto;padding:24px;color:#111;">
  <h2 style="font-size:18px;margin:0 0 8px;">
    BioEdge — ${count} product${count > 1 ? 's' : ''} waiting for <strong>${language}</strong> proofread
  </h2>
  ...
  <p style="margin-top:24px;">
    <a href="${FRONTEND_URL}/bioedge-proofread-queue"
      ...
      Open BioEdge Proofreading Queue →
    </a>
  </p>
  <p style="color:#aaa;font-size:11px;margin-top:24px;">
    You received this because you are assigned as a BioEdge ${language} proofreader.
  </p>
</div>`
