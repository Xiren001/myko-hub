...
exports.sendProofNotificationsForLanguage = sendProofNotificationsForLanguage;
...
async function sendProofNotificationsForLanguage(language) {
    const { data: products, error: prodErr } = await supabase_1.supabase
        .from('proof_products')
        ...
        .eq('done', false);
    if (prodErr)
        return { sent: false, count: 0, reason: prodErr.message };
    if (!products?.length)
        return { sent: false, count: 0, reason: 'no pending products' };
    const { data: emailConfig } = await supabase_1.supabase
        .from('proof_notification_emails')
        .select('emails')
        .eq('language', language)
        .maybeSingle();
    if (!emailConfig?.emails?.length)
        return { sent: false, count: products.length, reason: 'no emails configured' };
...
<div style="font-family:sans-serif;max-width:600px;margin:0 auto;padding:24px;color:#111;">
  ...
  <p style="color:#aaa;font-size:11px;margin-top:24px;">
    ...
    const results = await Promise.all(emailConfig.emails.map((to) => resend.emails.send({ from: FROM_EMAIL, to, subject, html })));
