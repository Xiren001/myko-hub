...
const generateWaveReportPdf_1 = require("../utils/generateWaveReportPdf");
...
function lpPhaseField(status) {
    const s = status.toLowerCase().trim();
    if (s.includes('building'))
        return 'lp_building_at';
    if (s === 'ready to launch')
        return 'lp_ready_to_launch_at';
    if (s === 'ready')
        return 'lp_ready_at';
    if (s.includes('proofread'))
        return 'lp_proofread_at';
    if (s === 'launched')
        return 'lp_launched_at';
    return null;
}
function parseWebhookValue(raw, field) {
    ...
    if (field.endsWith('_status')) {
        return val?.label?.text ?? null;
    }
    return val?.value ?? val?.text ?? (typeof val === 'string' ? val : null);
}
...
async function upsertProofProductFromSubitem(mondaySubitemId) {
    ...
    const variant = sub.name?.trim().toLowerCase() ?? '';
    if (!variant)
        return;
    if (variant === 'english' || variant === 'en' || variant === 'za')
        return;
    const productName = (sub.product_name ?? null);
    if (!productName)
        return;
    ...
    await supabase_1.supabase.from('proof_products').insert({
        ...
        language: null,
        pdp_url: (sub.page_link ?? null),
        drive_folder: (sub.drive_link ?? null),
        done: false,
...
router.post('/webhook', async (req, res) => {
    ...
    try {
        ...
        if (event.type === 'change_subitem_column_value') {
            ...
            const subPayload = { [field]: value, updated_at: new Date().toISOString() };
            await supabase_1.supabase.from('monday_subitems')
                .update(subPayload)
            ...
            const updatePayload = { [field]: value, updated_at: new Date().toISOString() };
            await supabase_1.supabase.from(table)
            ...
            const name = typeof event.value === 'string' ? event.value : event.value?.name;
            if (name) {
                const table = isSub ? 'monday_subitems' : 'monday_items';
                const idCol = isSub ? 'monday_subitem_id' : 'monday_item_id';
                await supabase_1.supabase.from(table)
            ...
            if (sub?.parent_item?.id) {
                const { data: parentItem } = await supabase_1.supabase.from('monday_items')
                ...
                if (parentItem) {
                    ...
                    for (const cv of sub.column_values ?? []) {
                        ...
                        if (f)
                            subCols[f] = BOOL_FIELDS.has(f) ? (cv.text === 'v' || cv.text === 'true') : (cv.text || null);
                    }
                    await supabase_1.supabase.from('monday_subitems').upsert({
            ...
            if (isSub) {
                await supabase_1.supabase.from('monday_subitems').delete().eq('monday_subitem_id', pulseId);
            }
            else {
                ...
                const { data: wave } = await supabase_1.supabase
                    .from('monday_waves').select('id').eq('board_id', boardId).single();
                if (wave) {
                    await supabase_1.supabase.from('monday_items').delete()
            ...
            const groupName = event.destGroup?.title ?? null;
            if (groupName) {
                await supabase_1.supabase.from('monday_items')
            ...
            if (currentItem) {
                ...
                const currentGroupName = currentItem.group?.title ?? null;
                const { data: wave } = await supabase_1.supabase
                    .from('monday_waves').select('id').eq('board_id', currentBoardId).single();
                if (wave) {
                    const { data: updated } = await supabase_1.supabase.from('monday_items')
...
async function fetchAndUpsertItem(itemId, boardId) {
    ...
    for (const cv of item.column_values ?? []) {
        const f = ITEM_COL[cv.id];
        if (f)
            itemCols[f] = cv.text || null;
    }
    const { data: ins } = await supabase_1.supabase.from('monday_items').upsert({
...
router.patch('/items/:itemId/timestamps', auth_1.authenticate, async (req, res) => {
    const { itemId } = req.params;
    const updates = {};
    for (const [key, val] of Object.entries(req.body)) {
        if (ALLOWED_TS_FIELDS.has(key))
            updates[key] = val || null;
    }
    if (!Object.keys(updates).length)
        return res.status(400).json({ error: 'No valid fields' });
    const { error } = await supabase_1.supabase.from('monday_items')
...
router.patch('/subitems/:subitemId/timestamps', auth_1.authenticate, async (req, res) => {
    ...
    if (!Object.keys(updates).length)
        return res.status(400).json({ error: 'No valid fields' });
    const { error } = await supabase_1.supabase.from('monday_subitems')
        .update({ ...updates, updated_at: new Date().toISOString() })
        .eq('id', subitemId);
    if (error)
        return res.status(500).json({ error: error.message });
...
router.post('/sync/:boardId', auth_1.authenticate, async (req, res) => {
    if (!MONDAY_TOKEN)
        return res.status(500).json({ error: 'MONDAY_API_TOKEN not set' });
    const { boardId } = req.params;
    const { data: wave, error: waveErr } = await supabase_1.supabase
        .from('monday_waves').select('id').eq('board_id', boardId).single();
    if (waveErr || !wave)
    ...
    try {
        ...
        const { data: dbItems } = await supabase_1.supabase
        ...
        for (const orphan of orphanItems) {
            await supabase_1.supabase.from('monday_subitems').delete().eq('item_id', orphan.id);
            await supabase_1.supabase.from('monday_items').delete().eq('id', orphan.id);
        }
        if (seenSubitemIds.length > 0) {
            const { data: dbSubs } = await supabase_1.supabase
            ...
            for (const orphan of orphanSubs) {
                await supabase_1.supabase.from('monday_subitems').delete().eq('id', orphan.id);
            }
        }
...
router.get('/waves', auth_1.authenticate, async (_req, res) => {
...
router.post('/import', auth_1.authenticate, auth_1.requireAdmin, async (_req, res) => {
    if (!MONDAY_TOKEN)
        return res.status(500).json({ error: 'MONDAY_API_TOKEN not set' });
    const boards = [
    ...
    for (const board of boards) {
        try {
            const { data: wave, error: wErr } = await supabase_1.supabase
                .from('monday_waves')
                .upsert({ wave_number: board.waveNumber, board_id: board.boardId, name: board.name }, { onConflict: 'board_id' })
                .select('id').single();
            if (wErr || !wave) {
                results[board.name] = `wave error: ${wErr?.message}`;
                continue;
            }
            let cursor = null;
            let count = 0;
...
router.post('/register-hooks', auth_1.authenticate, auth_1.requireAdmin, async (_req, res) => {
    if (!MONDAY_TOKEN)
        return res.status(500).json({ error: 'MONDAY_API_TOKEN not set' });
    const allBoards = [
...
router.post('/register-group-move-hooks', auth_1.authenticate, auth_1.requireAdmin, async (_req, res) => {
    if (!MONDAY_TOKEN)
        return res.status(500).json({ error: 'MONDAY_API_TOKEN not set' });
    const results = {};
...
router.post('/register-item-move-hooks', auth_1.authenticate, auth_1.requireAdmin, async (_req, res) => {
    if (!MONDAY_TOKEN)
        return res.status(500).json({ error: 'MONDAY_API_TOKEN not set' });
    const results = {};
    for (const boardId of Object.keys(PARENT_BOARD_MAP)) {
...
router.post('/register-crud-hooks', auth_1.authenticate, auth_1.requireAdmin, async (_req, res) => {
    if (!MONDAY_TOKEN)
        return res.status(500).json({ error: 'MONDAY_API_TOKEN not set' });
    ...
    for (const boardId of Object.keys(PARENT_BOARD_MAP)) {
      ...
      `);
            results[boardId][event] = resp?.data?.create_webhook ?? resp?.errors;
        }
    }
    return res.json({ ok: true, results });
});
// ── POST /api/monday/language-sales/upload ───────────────────────────────
router.post('/language-sales/upload', auth_1.authenticate, express_1.default.text({ type: '*/*', limit: '5mb' }), async (req, res) => {
    ...
    for (const line of lines.slice(1)) {
        ...
        if (!lang)
            continue;
        if (!langAgg[lang])
            langAgg[lang] = { country, net_sales: 0, cogs: 0 };
    ...
    await supabase_1.supabase.from('language_sales').delete().neq('lang_code', '');
    const { error } = await supabase_1.supabase.from('language_sales').insert(rows);
    if (error)
        return res.status(500).json({ error: error.message });
    res.json({ ok: true, uploaded: rows.length });
});
// ── POST /api/monday/product-sales/upload ────────────────────────────────
router.post('/product-sales/upload', auth_1.authenticate, express_1.default.text({ type: '*/*', limit: '5mb' }), async (req, res) => {
    const csvText = req.body;
    const lines = csvText.split(/\r?\n/).filter(l => l.trim());
    const rows = [];
    ...
    await supabase_1.supabase.from('product_sales').delete().neq('product_title', '');
    const { error } = await supabase_1.supabase.from('product_sales').insert(rows);
    if (error)
        return res.status(500).json({ error: error.message });
    res.json({ ok: true, uploaded: rows.length });
});
// ── GET /api/monday/waves-weekly-report ──────────────────────────────────
router.get('/waves-weekly-report', auth_1.authenticate, async (req, res) => {
...
router.post('/wave-report-snapshot', auth_1.authenticate, auth_1.requireAdmin, async (_req, res) => {
    try {
        await (0, waveReportCron_1.runWaveReportSnapshot)();
        return res.json({ ok: true });
    }
    catch (err) {
        return res.status(500).json({ error: err.message });
...
router.get('/wave-report-snapshot/:weekStart/pdf', auth_1.authenticate, async (req, res) => {
    ...
    if (snap) {
        ...
        reportData = await (0, computeWavesReport_1.computeWavesReport)();
    }
    try {
        const pdfBuffer = await (0, generateWaveReportPdf_1.generateWaveReportPdf)(reportData, isSnapshot);
...
router.get('/wave-report-cron', auth_1.authenticate, async (_req, res) => {
    const schedule = await (0, waveReportCron_1.getCronSchedule)();
    return res.json(schedule);
});
// ── PUT /api/monday/wave-report-cron ──────────────────────────────────────
router.put('/wave-report-cron', auth_1.authenticate, auth_1.requireAdmin, async (req, res) => {
