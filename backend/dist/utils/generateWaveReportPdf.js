...
exports.generateWaveReportPdf = generateWaveReportPdf;
...
function waves27Metrics(d) {
    const validAvgs = d.newWaveCampaignAvgDays.map(r => r.avg).filter((v) => v !== null);
...
function generateWaveReportPdf(data, isSnapshot) {
    return new Promise((resolve, reject) => {
        const doc = new pdfkit_1.default({ size: 'A4', margin: 0, info: {
        ...
        doc.font('Helvetica').fontSize(8).fillColor('#64748b')
            .text(`Generated ${new Date().toLocaleString('en-GB', { dateStyle: 'medium', timeStyle: 'short' })}`, MARGIN, 86, { width: BODY_W });
        y = 126;
        // ── SECTION RENDERER ─────────────────────────────────────────────────────
        ...
        for (const row of wave1Metrics(data))
            metricRow(row);
        y += 4;
        sectionHeader('Waves 2–7');
        for (const row of waves27Metrics(data))
            metricRow(row);
        y += 4;
        teamQueueSection();
        // ── FOOTER ────────────────────────────────────────────────────────────────
